...
var createEncoder2 = () => new Encoder();
var length = (encoder) => {
...
var _writeVarStringNative = (encoder, str) => {
  if (str.length < _maxStrBSize) {
    ...
    writeVarUint8Array2(encoder, encodeUtf8(str));
  }
};
...
var writeVarString2 = utf8TextEncoder && /** @type {any} */
...
var createDecoder2 = (uint8Array) => new Decoder(uint8Array);
var readUint8Array = (decoder, len) => {
  const view = new Uint8Array(decoder.arr.buffer, decoder.pos + decoder.arr.byteOffset, len);
  decoder.pos += len;
  return view;
};
var readVarUint8Array2 = (decoder) => readUint8Array(decoder, readVarUint2(decoder));
...
messageHandlers[messageSync] = (encoder, decoder, provider, emitSynced, _messageType) => {
  writeVarUint2(encoder, messageSync);
...
messageHandlers[messageQueryAwareness] = (encoder, _decoder, provider, _emitSynced, _messageType) => {
  writeVarUint2(encoder, messageAwareness);
  writeVarUint8Array2(
    encoder,
...
var readMessage = (provider, buf, emitSynced) => {
  const decoder = createDecoder2(buf);
...
var setupWS = (provider) => {
  if (provider.shouldConnect && provider.ws === null) {
    ...
    websocket.onmessage = (event) => {
      ...
      if (length(encoder) > 1) {
        websocket.send(toUint8Array2(encoder));
      }
    };
  ...
  constructor(serverUrl, roomname, doc, {
    ...
    if (resyncInterval > 0) {
      this._resyncInterval = setInterval(() => {
        if (this.ws && this.ws.readyState === WebSocket.OPEN) {
          ...
          this.ws.send(toUint8Array2(encoder));
        }
      }, resyncInterval);
    }
    this._bcSubscriber = (data, origin) => {
      if (origin !== this) {
        ...
        if (length(encoder) > 1) {
          publish(this.bcChannel, toUint8Array2(encoder), this);
        }
      }
    };
    this._updateHandler = (update, origin) => {
      if (origin !== this) {
        ...
        broadcastMessage(this, toUint8Array2(encoder));
      }
    };
...
//# sourceMappingURL=@y-sweet_client.js.map
